...
MongoLogger::MongoLogger(const char* databaseIP, std::uint32_t databasePort, const char* databaseName)
...
bool MongoLogger::logMethod(const char* method, std::uint64_t clientID, const char* logContent)
   {
   struct timespec t;
